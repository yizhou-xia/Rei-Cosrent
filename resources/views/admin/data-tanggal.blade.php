...
@section('title', 'Data Tanggal Pemesanan - Rei Cosrent')
    ...
    <div class="container">
        ...
        <div class="booking-calendar-section">
            <div class="booking-calendar-heading py-3">
                <h5 class="fw-bold mb-0">{{ $namaBulan[$selectedMonth] }} {{ $selectedYear }}</h5>
                <div class="booking-month-navigation" aria-label="Navigasi bulan kalender">
                    <a href="{{ route('admin.data-tanggal', ['year' => $previousMonth->year, 'month' => $previousMonth->month]) }}" class="btn btn-sm btn-outline-secondary" title="Bulan sebelumnya" aria-label="Bulan sebelumnya">
                        <i class="bi bi-chevron-left me-1" aria-hidden="true"></i> Sebelumnya
                    </a>
                    <a href="{{ route('admin.data-tanggal', ['year' => $nextMonth->year, 'month' => $nextMonth->month]) }}" class="btn btn-sm btn-outline-secondary" title="Bulan berikutnya" aria-label="Bulan berikutnya">
            ...
            <div class="p-0">
                <div class="table-responsive booking-calendar-table-wrap">
                    <table class="table table-hover align-middle mb-0 orders-table booking-calendar-table" style="width: {{ $calendarTableWidth }}px; max-width: none;">
                        <colgroup>
                        ...
                        <thead>
                            <tr>
                                ...
                                @foreach($dates as $d)
                                    <th style="min-width: 90px;">{{ \Carbon\Carbon::parse($d)->format('d') }}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($kostums as $index => $k)
                                <tr class="booking-calendar-row">
                                    <td class="booking-index-cell">{{ $index + 1 }}</td>
                                    <td class="booking-costume-cell">{{ $k->nama_kostum }}</td>
                                    @foreach($dates as $d)
                                        <td>
                                            @if(isset($bookingMap[$k->nama_kostum][$d]))
                                                @php($booking = $bookingMap[$k->nama_kostum][$d])
                                                <a href="{{ route('admin.data-pesanan', ['open_detail' => $booking['order_id']]) }}" class="booking-slot-badge {{ $booking['status_class'] }} text-decoration-none" title="{{ $booking['username'] }} - Lihat detail pesanan" aria-label="{{ $booking['username'] }} - Lihat detail pesanan">
                                                    <span class="booking-slot-label">{{ $booking['username'] }}</span>
                                                </a>
                                            @else
