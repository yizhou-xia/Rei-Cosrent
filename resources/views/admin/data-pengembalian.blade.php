                ...
                <table id="adminPengembalianTable" class="table table-hover align-middle orders-table">
                    ...
                    <tbody>
                        @foreach($pengembalianList as $index => $item)
                                ...
                                @endphp
<tr data-date="{{ $item->created_at ? $item->created_at->toDateString() : '' }}">
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#imgModal-{{ $item->id }}">
                                ...
                                <div class="modal fade" id="imgModal-{{ $item->id }}" tabindex="-1" aria-hidden="true">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="row g-3 mb-3 text-start">
                                                        ...
                                                        <div class="col-md-4">
                                                            ...
                                                            <div class="fw-semibold">{{ $item->created_at ? $item->created_at->format('d M Y H:i') : '-' }}</div>
                                                        </div>
                                                        <div class="col-md-4">
                                                            <div class="small text-muted mb-1">Status</div>
                                                            <div><span class="badge status-badge {{ $statusModalData[1] }}">{{ $statusModalData[0] }}</span></div>
                                                        </div>
                                                        <div class="col-md-4">
                                                        ...
                                                        <div class="row g-3 mb-3">
                                                            @foreach($buktiList as $index => $bukti)
                                                                <div class="col-6 col-md-4 text-center">
                                                                    <button type="button" class="btn p-0 border-0 bg-transparent return-photo-detail-trigger" data-photo-src="{{ asset('storage/' . $bukti) }}" data-photo-title="Bukti Pengembalian {{ $index + 1 }}" aria-label="Lihat detail bukti pengembalian {{ $index + 1 }}">
                                                                        <img src="{{ asset('storage/' . $bukti) }}" alt="Bukti {{ $index + 1 }}" class="img-fluid history-thumb">
                                                                    </button>
                                                                    <small class="d-block mt-2">Bukti {{ $index + 1 }}</small>
                                                                </div>
                                                            @endforeach
                                                        </div>
                                                    @else
                                                    ...
                                                    <div class="text-start mt-3">
                                                        ...
                                                        <div class="text-muted">{{ $item->catatan ?: 'Tidak ada catatan.' }}</div>
                                                        <hr>
                                                        <div class="fw-semibold mb-2">Catatan Admin</div>
                                                        <div class="text-muted">{{ $catatanAdmin === '-' ? 'Belum ada catatan admin.' : $catatanAdmin }}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
