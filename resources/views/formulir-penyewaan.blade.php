...
@section('title', 'Formulir Penyewaan - Rei Cosrent')
...
<section class="py-4">
    <div class="container">
            ...
            <div class="kostum-info">
                <div class="kostum-info-layout">
                    <div class="kostum-image-wrap">
                        ...
                        @endphp
                        @if($src)
                            <img src="{{ $src }}" alt="{{ $kostum->nama_kostum }}" class="kostum-image rounded">
                        @else
                            <img src="{{ asset('assets/img/no-image.png') }}" alt="Tidak ada gambar" class="kostum-image rounded">
                        @endif
                    </div>
                    <div class="kostum-details">
                        <h5 class="fw-bold">{{ $kostum->nama_kostum }}</h5>
                        @if($kostum->judul)
                            <p class="text-muted mb-2">{{ $kostum->judul }}</p>
                        @endif
        ...
        @endphp
        <div id="rentalAvailabilityAlert" class="alert alert-warning d-flex align-items-center {{ $formAlertMessage ? '' : 'd-none' }}" role="alert" aria-live="polite" data-server-alert="{{ $formAlertMessage ? '1' : '0' }}">
            <i class="bi bi-clock-history me-2"></i>
            <span id="rentalAvailabilityMessage">{{ $formAlertMessage }}</span>
        </div>

        <!-- Form Penyewaan -->
        <form method="POST" action="{{ route('formulir.penyewaan.submit') }}" enctype="multipart/form-data">
            @csrf
                            <input type="hidden" name="id_kostum" value="{{ $kostum->id_kostum }}">
            <input type="hidden" name="nama_kostum" value="{{ $kostum->nama_kostum }}">
            ...
            <div class="form-section">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="username" class="form-control" value="{{ old('username', $currentUser->username ?? '') }}" required readonly autocomplete="username">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label required-label">Nama Lengkap</label>
                        <input type="text" name="nama" class="form-control" value="{{ old('nama', $currentUser->nick_name ?? '') }}" required readonly autocomplete="name">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label required-label">Nomor Telepon</label>
                        <input type="text" name="nomor_telepon" class="form-control" value="{{ old('nomor_telepon', $currentUser->nomor_telepon ?? '') }}" required readonly autocomplete="tel">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label required-label">Nomor Telepon Pihak Kedua (Orang Tua/Wali/Tetangga/DLL)</label>
                        <input type="text" name="nomor_telepon_2" class="form-control" value="{{ old('nomor_telepon_2') }}" required placeholder="Contoh: 08xxx - Orang Tua" autocomplete="off">
                    </div>
                    @if(session('user_logged_in'))
                        <div class="col-md-12">
                            <label class="form-label required-label" for="alamat">Alamat</label>
                            <input type="text" name="alamat" id="alamat" class="form-control" value="{{ old('alamat', $alamatUser) }}" required readonly>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" name="tanggal_pemakaian" class="form-control" value="{{ old('tanggal_pemakaian') }}" min="{{ now()->toDateString() }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label required-label">Tanggal Pengembalian</label>
                        <input type="date" name="tanggal_pengembalian" class="form-control" value="{{ old('tanggal_pengembalian') }}" min="{{ now()->toDateString() }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label required-label">Harga Sewa</label>
                        <input type="number" name="harga_sewa" id="harga_sewa" class="form-control" value="{{ old('harga_sewa', $kostum->harga_sewa) }}" readonly>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label required-label">Ongkir</label>
                        <input type="number" name="ongkir" id="ongkir" class="form-control" value="{{ old('ongkir', 0) }}" readonly>
                    </div>
                    @if(filled($kostum->exclude) && !is_null($kostum->harga_exclude) && $kostum->is_exclude_active)
                        <div class="col-md-12">
                            <div class="form-check form-switch p-3 rounded border" style="border-color: var(--rental-control-border) !important; background: var(--rental-costume-bg);">
                                <input class="form-check-input ms-0 me-2" type="checkbox" role="switch" name="sewa_exclude" id="sewa_exclude" value="1" {{ old('sewa_exclude') ? 'checked' : '' }} data-harga-exclude="{{ $kostum->harga_exclude }}">
                                <label class="form-check-label fw-semibold" for="sewa_exclude">Sewa dengan Exclude (+Rp {{ number_format((float) $kostum->harga_exclude, 0, ',', '.') }})</label>
                            </div>
                        </div>
                    @endif
                    <div class="col-md-12">
                        <label class="form-label required-label">Total Harga</label>
                        <input type="number" name="total_harga" id="total_harga" class="form-control" value="{{ old('total_harga') }}" required min="0" step="0.01" readonly>
                    </div>
                    <div class="col-md-12">
                        ...
                        <div class="row g-3">
                            <div class="col-12 col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="metode_pembayaran" id="pembayaran_qris" value="QRIS" {{ old('metode_pembayaran') === 'QRIS' ? 'checked' : '' }} required>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="metode_pembayaran" id="pembayaran_cod" value="COD" {{ old('metode_pembayaran') === 'COD' ? 'checked' : '' }} required>
                ...
                <div class="row g-3">
                    <div class="col-md-12">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="kartu_identitas" id="identitas_pelajar" value="Kartu Pelajar" {{ old('kartu_identitas') === 'Kartu Pelajar' ? 'checked' : '' }} required>
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="kartu_identitas" id="identitas_kia" value="KIA" {{ old('kartu_identitas') === 'KIA' ? 'checked' : '' }} required>
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="kartu_identitas" id="identitas_ktm" value="KTM" {{ old('kartu_identitas') === 'KTM' ? 'checked' : '' }} required>
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="kartu_identitas" id="identitas_ktp" value="KTP" {{ old('kartu_identitas') === 'KTP' ? 'checked' : '' }} required>
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="kartu_identitas" id="identitas_lainnya" value="Lainnya" {{ old('kartu_identitas') === 'Lainnya' ? 'checked' : '' }} required>
...
3. Menerima Konsekuensi`;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function fetchOngkir() {
            ...
            fetch('{{ route('rajaongkir.shipping-cost') }}?address=' + encodeURIComponent(alamatInput.value.trim()))
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const rentalForm = document.querySelector('form[action="{{ route('formulir.penyewaan.submit') }}"]');
        const alamatInput = document.querySelector('input[name="alamat"]');
        ...
        function checkCostumeAvailability() {
            ...
            const params = new URLSearchParams({
                id_kostum: '{{ $kostum->id_kostum }}',
                tanggal_pemakaian: startDate,
                tanggal_pengembalian: endDate,
            });

            fetch('{{ route('formulir.penyewaan.availability') }}?' + params.toString(), {
