...
@section('title', 'Tanggal Pemesanan - Rei Cosrent')
        ...
        <div class="booking-calendar-section">
            <div class="booking-calendar-heading py-3">
                ...
                <div class="booking-month-navigation" aria-label="Navigasi bulan kalender">
                    <a href="{{ route('tanggal.pemesanan', ['year' => $previousMonth->year, 'month' => $previousMonth->month]) }}" class="btn btn-sm btn-outline-secondary" title="Bulan sebelumnya" aria-label="Bulan sebelumnya">
                        <i class="bi bi-chevron-left me-1" aria-hidden="true"></i> Sebelumnya
                    </a>
                    <a href="{{ route('tanggal.pemesanan', ['year' => $nextMonth->year, 'month' => $nextMonth->month]) }}" class="btn btn-sm btn-outline-secondary" title="Bulan berikutnya" aria-label="Bulan berikutnya">
            ...
            <div class="p-0">
                <div class="booking-table-wrap booking-calendar-table-wrap">
                    <table class="table table-hover align-middle mb-0 booking-table booking-calendar-table" style="width: {{ $calendarTableWidth }}px; max-width: none;">
                        <colgroup>
                                    ...
                                    @foreach($dates as $d)
                                        <td>
                                            @if(isset($bookingMap[$k->nama_kostum][$d]))
                                                ...
                                                <span class="booking-slot-badge {{ $booking['status_class'] }}" title="{{ $booking['username'] }}" aria-label="{{ $booking['username'] }}"><span class="booking-slot-label">{{ $booking['username'] }}</span></span>
                                            @else
